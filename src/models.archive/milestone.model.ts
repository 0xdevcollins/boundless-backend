...
MilestoneSchema.methods.canTransitionTo = function (
  ...
  const validTransitions: Record<MilestoneStatus, MilestoneStatus[]> = {
    ...
    [MilestoneStatus.COMPLETED]: [MilestoneStatus.RELEASED],
    [MilestoneStatus.RELEASED]: [],
    [MilestoneStatus.OVERDUE]: [
      MilestoneStatus.IN_PROGRESS,
      MilestoneStatus.CANCELLED,
...
export function validateMilestoneForReview(milestone: IMilestone): {
  isValid: boolean;
  error?: string;
} {
  if (!milestone.isReviewable()) {
    return {
      isValid: false,
      error: `Milestone with status '${milestone.status}' cannot be reviewed. Only milestones with status '${REVIEWABLE_STATES.join(
        "', '",
      )}' can be reviewed.`,
    };
  }
